...
required_packages <- c(
  "shiny", "readxl", "dplyr", "DT", "FactoMineR", "factoextra", 
  "colourpicker", "ggplot2", "corrplot", "plotly", "grid", "gridExtra", 
  "reshape2", "cluster", "heatmaply", "dendextend", "viridis", 
  "RColorBrewer", "apcluster", "dbscan", "kernlab", "writexl"
)
...
ui <- navbarPage("Data Analysis App",
                          ...
                          fluidPage(
                            ...
                            tags$ul(
                              ...
                              tags$li(
                                ...
                                tags$img(src = "https://upload.wikimedia.org/wikipedia/commons/8/89/Boxplot_vs_PDF.png", 
                                         alt="Box Plot Explanation Diagram", style="width:600px; height:400px;")
                              ),
                              tags$li(
                 ...
                 # New Dataset Tab: Import and preprocess data
                          ...
                          sidebarLayout(
                            ...
                            mainPanel(
                              tabsetPanel(
                                ...
                                tabPanel("PC Matrix", DTOutput("pc_matrix"), br(), downloadButton("download_pc_matrix", "Download PC Matrix as Excel"))
                              )
                            )
                          )
                 ),
                          ...
                          sidebarLayout(
                            sidebarPanel(
                              ...
                              selectInput("distance_method", "Distance Method:",
                                          choices = c("euclidean", "manhattan", "maximum", "minkowski"), selected = "euclidean"),
                              conditionalPanel(
                            ...
                            mainPanel(
                              tabsetPanel(
                                tabPanel("Elbow Analysis", br(), plotOutput("elbow_plot"), br(), verbatimTextOutput("optimal_clusters")),
                                tabPanel("K-means Analysis", br(), plotOutput("kmeans_plot"), hr(), h4("Cluster Summary Statistics"), DTOutput("cluster_summary"), hr(), h4("Sample Details"), DTOutput("sample_cluster_details"))
                              )
                            )
                          )
                 ),
...
server <- function(input, output, session) {
  ...
  rawData <- eventReactive(input$loadData, {
    ...
    if(grepl("\\.csv$", input$file$name, ignore.case = TRUE)) {
      ...
      readxl::read_excel(filepath, sheet = sheets[1], skip = input$headerRow - 1)
    }
  })
  ...
  dataImport <- reactive({
    ...
    if (!is.null(input$sheet)) {
      readxl::read_excel(input$file$datapath, sheet = input$sheet, skip = input$headerRow - 1)
    } else {
      rawData()
    }
  })
  ...
  output$orig_data <- renderDT({
    req(dataImport())
    DT::datatable(dataImport(), options = list(scrollX = TRUE))
  })
  
  # UI Elements for Dataset Creation
  output$id_col_ui <- renderUI({
    req(dataImport())
    selectInput("id_col", "Select ID Column:", choices = names(dataImport()), selected = names(dataImport())[1])
  })
  
  output$group_col_ui <- renderUI({
    req(dataImport())
    selectInput("group_col", "Select Grouping Column (Labels):", choices = names(dataImport()), selected = names(dataImport())[1])
  })
  
  output$select_vars_ui <- renderUI({
    req(dataImport())
    selectizeInput("selected_vars", "Select Variables to include:", choices = names(dataImport()), multiple = TRUE)
  })
  ...
  newDatabase <- eventReactive(input$createDb, {
    ...
    if (input$remove_outliers) {
      newdf <- newdf %>% dplyr::group_by_at(input$group_col) %>%
        dplyr::filter(if_all(all_of(input$selected_vars), function(x) {
  ...
  output$new_data <- renderDT({
    req(newDatabase())
    DT::datatable(newDatabase(), options = list(scrollX = TRUE))
  })
  
  output$downloadData <- downloadHandler(
    filename = function() { paste("new_dataset-", Sys.Date(), ".csv", sep = "") },
    content = function(file) { write.csv(newDatabase(), file, row.names = FALSE) }
  )
  ...
  filtered_data <- reactive({
    req(newDatabase(), input$pca_group_filter)
    dat <- newDatabase()
  ...
  pcaResult <- reactive({
    ...
    dat <- filtered_data()
    pca_data <- dat %>% dplyr::select(all_of(input$pca_vars))
    complete_idx <- complete.cases(pca_data)
    valid_ids <- dat$ID[complete_idx]
    pca_data <- pca_data[complete_idx, , drop=FALSE]
    result <- FactoMineR::PCA(pca_data, scale.unit=TRUE, graph=FALSE)
    rownames(result$ind$coord) <- valid_ids
    result
  })
  ...
  output$pcaCombinedPlot <- renderPlot({
    ...
    p2 <- ggplot2::ggplot(selected_loadings, ggplot2::aes(x=x, y=y, label=Variable)) +
      ggplot2::geom_segment(ggplot2::aes(x=0, y=0, xend=x, yend=y), arrow=grid::arrow(length=grid::unit(0.2, "cm")), color="grey50") +
      ggplot2::geom_text(color="black", hjust=-0.2) +
      ggplot2::coord_fixed() +
      ggplot2::labs(title="PCA Loadings Plot", x=input$pca_xaxis, y=input$pca_yaxis) +
  ...
  output$pcaContributions <- renderPlot({
    ...
    xAxis <- as.numeric(gsub("PC", "", input$pca_xaxis))
    factoextra::fviz_contrib(pcaResult(), choice="var", axes=xAxis, top=10) +
      ggplot2::theme_minimal() +
      ggplot2::theme(axis.title=ggplot2::element_text(size=input$pca_axis_title_size),
                     axis.text=ggplot2::element_text(size=input$pca_axis_text_size))
  })
  ...
  output$variableCorrelation <- renderPlot({
    ...
    corrplot::corrplot(cor_matrix, method="color", type="upper", order="hclust",
                       ...
                       col=colorRampPalette(c("blue", "white", "red"))(100), diag=FALSE,
                       title="Variable Correlation Matrix", mar=c(0,0,1,0))
  })
  ...
  output$sampleCorrelation <- renderPlotly({
    ...
    plotly::plot_ly(x=input$pca_vars, y=dat$ID, z=sample_var_cor, type="heatmap",
                    colors = colorRamp(c("green", "white", "blue")),
                    text=matrix(sprintf("%.2f", sample_var_cor), ncol=ncol(sample_var_cor))) %>%
      plotly::layout(title="Sample-Variable Correlation Matrix",
                     xaxis=list(title="Variables", titlefont=list(size=input$pca_axis_title_size),
                                tickfont=list(size=input$pca_axis_text_size), tickangle=45),
                     yaxis=list(title="Samples", titlefont=list(size=input$pca_axis_title_size),
                                tickfont=list(size=input$pca_axis_text_size)))
  })
  
  output$download_pca_plot <- downloadHandler(
    filename = function() { paste("pca_plot.", input$pca_download_format, sep="") },
    content = function(file) {
      tryCatch({
        if(input$pca_download_format == "pdf") {
          ...
          jpeg(file, width=input$pca_download_width*input$pca_download_dpi, height=input$pca_download_height*input$pca_download_dpi,
               res=input$pca_download_dpi, quality=100)
        } else if(input$pca_download_format == "tiff") {
          tiff(file, width=input$pca_download_width*input$pca_download_dpi, height=input$pca_download_height*input$pca_download_dpi,
        ...
        p2 <- ggplot2::ggplot(selected_loadings, ggplot2::aes(x=x, y=y, label=Variable)) +
          ggplot2::geom_segment(ggplot2::aes(x=0, y=0, xend=x, yend=y),
                                arrow=grid::arrow(length=grid::unit(0.2,"cm")), color="grey50") +
          ggplot2::geom_text(color="black", hjust=-0.2) +
          ggplot2::coord_fixed() +
          ggplot2::labs(title="PCA Loadings Plot", x=input$pca_xaxis, y=input$pca_yaxis) +
  ...
  output$download_contrib_plot <- downloadHandler(
    filename = function() { paste("contribution_plot.", input$pca_download_format, sep="") },
    content = function(file) {
      tryCatch({
        if(input$pca_download_format=="pdf") {
          pdf(file, width=input$pca_download_width, height=input$pca_download_height)
        ...
        xAxis <- as.numeric(gsub("PC", "", input$pca_xaxis))
        print(factoextra::fviz_contrib(pcaResult(), choice="var", axes=xAxis, top=10) +
                ggplot2::theme_minimal() +
                ggplot2::theme(axis.title=ggplot2::element_text(size=input$pca_axis_title_size),
                               axis.text=ggplot2::element_text(size=input$pca_axis_text_size)))
        dev.off()
  ...
  output$download_corr_plot <- downloadHandler(
    filename = function() { paste("correlation_plot.", input$pca_download_format, sep="") },
    content = function(file) {
      tryCatch({
        if(input$pca_download_format=="pdf") {
          pdf(file, width=input$pca_download_width, height=input$pca_download_height)
  ...
  output$download_pc_matrix <- downloadHandler(
    filename = function() { paste("pc_matrix-", Sys.Date(), ".xlsx", sep="") },
    content = function(file) {
      req(pcaResult())
      dat <- as.data.frame(pcaResult()$ind$coord)
      dat$ID <- rownames(dat)
  ...
  output$violinPlot <- renderPlot({
    ...
    if(input$show_boxplot) { p <- p + ggplot2::geom_boxplot(width=0.2, fill="white", alpha=0.5, outlier.shape=NA) }
    if(input$show_points) { p <- p + ggplot2::geom_jitter(width=0.1, alpha=0.5) }
    p
  })
  
  output$download_violin_plot <- downloadHandler(
    filename = function() { paste("violin_plot.", input$violin_download_format, sep="") },
    content = function(file) {
      tryCatch({
        if(input$violin_download_format=="pdf") {
          ...
          jpeg(file, width=input$violin_download_width*input$violin_download_dpi, height=input$violin_download_height*input$violin_download_dpi,
               res=input$violin_download_dpi, quality=100)
        } else if(input$violin_download_format=="tiff") {
          tiff(file, width=input$violin_download_width*input$violin_download_dpi, height=input$violin_download_height*input$violin_download_dpi,
  ...
  output$kmeans_vars_ui <- renderUI({
    ...
    dat <- newDatabase()
    num_vars <- names(dat)[sapply(dat, is.numeric)]
    selectizeInput("kmeans_vars", "Select variables for clustering:", choices = num_vars, multiple = TRUE)
  })
  
  output$kmeans_group_select_ui <- renderUI({
    req(newDatabase())
    groups <- unique(newDatabase()[[input$group_col]])
    selectizeInput("kmeans_groups_selected", "Select groups for analysis:", choices = groups, multiple = TRUE, selected = groups)
  })
  
  output$kmeans_group_filter_ui <- renderUI({
    req(kmeans_result())
    clusters <- sort(unique(kmeans_result()$cluster))
    checkboxGroupInput("kmeans_group_filter", "Select clusters to display:", choices = clusters, selected = clusters)
  })
  
  kmeans_data <- reactive({
    req(newDatabase(), input$kmeans_vars, input$kmeans_groups_selected)
    filtered_data <- newDatabase()[newDatabase()[[input$group_col]] %in% input$kmeans_groups_selected, ]
  ...
  perform_clustering <- function(data, k, distance_method, power=2) {
    if(distance_method=="euclidean") {
      ...
      list(cluster=pam_result$clustering, centers=centers, size=pam_result$clusinfo[,"size"], withinss=pam_result$objective)
    }
  }
  ...
  output$elbow_plot <- renderPlot({
    req(elbow_data())
    ggplot2::ggplot(elbow_data(), ggplot2::aes(x=k, y=wss)) +
      ggplot2::geom_line() +
      ggplot2::geom_point() +
      ggplot2::labs(x="Number of Clusters (k)", y="Total Within Sum of Squares",
                    title=paste("Elbow Method for Optimal k Selection\nusing", input$distance_method, "distance")) +
      ggplot2::theme_minimal() +
      ggplot2::theme(axis.title=ggplot2::element_text(size=input$kmeans_axis_title_size),
                     axis.text=ggplot2::element_text(size=input$kmeans_axis_text_size),
  ...
  output$kmeans_plot <- renderPlot({
    ...
    p <- ggplot2::ggplot(data_plot, ggplot2::aes_string(x=xvar, y=yvar, color="Cluster")) +
      ggplot2::geom_point(data=subset(data_plot, !Selected), color="grey80", alpha=0.3) +
      ggplot2::geom_point(data=subset(data_plot, Selected), alpha=0.6) +
      ggplot2::stat_ellipse(data=subset(data_plot, Selected), type="norm", level=0.95) +
      ggplot2::scale_color_brewer(palette="Set1") +
      ggplot2::labs(title=paste("Clustering with", input$distance_method, "distance"),
                    x=input$kmeans_vars[1], y=input$kmeans_vars[2]) +
      ggplot2::theme_minimal() +
      ggplot2::theme(axis.title=ggplot2::element_text(size=input$kmeans_axis_title_size),
                     axis.text=ggplot2::element_text(size=input$kmeans_axis_text_size),
    ...
    p + ggplot2::geom_point(data=centers_df,
  ...
  output$cluster_summary <- renderDT({
    req(kmeans_result(), newDatabase())
    filtered_data_clust <- newDatabase()[newDatabase()[[input$group_col]] %in% input$kmeans_groups_selected, ]
    cluster_data <- data.frame(filtered_data_clust[, input$kmeans_vars, drop=FALSE],
                               Cluster=as.factor(kmeans_result()$cluster))
    summary_stats <- cluster_data %>%
      dplyr::group_by(Cluster) %>%
      dplyr::summarise(across(everything(), list(mean=~mean(.), sd=~sd(.)), .names="{.col}_{.fn}"),
                       Size=n())
  ...
  output$sample_cluster_details <- renderDT({
    ...
    DT::datatable(sample_data, options=list(scrollX=TRUE, pageLength=15, dom='Bfrtip', buttons=c('copy','csv','excel')),
                  rownames=FALSE) %>% 
      DT::formatStyle('Selected', target='row', backgroundColor=DT::styleEqual(c(TRUE,FALSE), c('#fff3cd','white')))
  })
  ...
  output$download_clustered_data <- downloadHandler(
    filename = function() { paste("clustered_dataset-", Sys.Date(), ".csv", sep="") },
    content = function(file) { write.csv(clusteredData(), file, row.names=FALSE) }
  )
  
  output$download_elbow_plot <- downloadHandler(
    filename = function() { paste("elbow_plot", input$kmeans_download_format, sep=".") },
    content = function(file) {
      ggplot2::ggsave(file,
                      plot = {
                        ggplot2::ggplot(elbow_data(), ggplot2::aes(x=k, y=wss)) +
  ...
  output$download_kmeans_plot <- downloadHandler(
    filename = function() { paste("kmeans_plot", input$kmeans_download_format, sep=".") },
    content = function(file) {
      ggplot2::ggsave(file,
                      plot = {
                        data_plot <- as.data.frame(kmeans_data())
  ...
  # Complete Dataset Section: Merge datasets, PCA and clustering results
  completeDataset <- reactive({
    validate(
      need(!is.null(newDatabase()), "Please generate the new dataset"),
      need(!is.null(pcaResult()), "Please run PCA analysis"),
      need(!is.null(clusteredData()), "Please run clustering analysis")
    )
    new_ds <- newDatabase()[, c("ID", input$group_col, input$selected_vars)]
  ...
  observeEvent(input$generateCompleteData, {
    output$complete_data <- renderDT({
      req(completeDataset())
      DT::datatable(completeDataset(), options=list(scrollX=TRUE))
    })
  })
  
  output$download_complete_data <- downloadHandler(
    filename = function() { paste("complete_dataset-", Sys.Date(), ".xlsx", sep="") },
    content = function(file) {
      req(completeDataset())
